...
model.add(tf.keras.layers.Dense(4, activation=activation_functions.activation_funcs[2]))
...
log_dir = "./logs/fit/"+"final2" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)

model.fit(training2.training_inputs,training2.training_outputs,epochs=50,validation_data=(test2.test_inputs,test2.test_outputs),callbacks=[tensorboard_callback])
